...
bool CLanguageLocale::SetLanguage(int nID)
{
	...
	else
	{
		...
		while(bRet)
		{
			...
			// 也尝试通过文件名匹配（如 LanguageLocale_zh.ini）
			CString fileName = find.GetFileName();
			if (fileName.Find(_T("_zh")) >= 0 && nID == LANGUAGE_CHINESE) {
				gStrFileName = szFilePath;
				bFound = true;
				break;
			}
			if (fileName.Find(_T("_en")) >= 0 && nID == LANGUAGE_ENGLISH) {
				gStrFileName = szFilePath;
				bFound = true;
				break;
			}
...
{ 
    ...
    nCount =GetMenuItemCount(menu);// menu->GetMenuItemCount();   //��ȡ�ò�˵��� 
    for(int i=0;i<nCount;i++)                //�������в˵��� 
    { 
        //�鿴�Ӳ˵�   ������Ӳ˵�������0�����򷵻��Ӳ˵�����Ŀ��������Ӳ˵��������Դ�еĲ˵�COMMAND ID 
		 KeyName=_T("");
		 KeyName=tempname;
...
{ 
	...
	nCount= menu->GetMenuItemCount();    //��ȡ�ò�˵��� 

	for(int i=0;i<nCount;i++ )               //�������в˵��� 
	{ 
		menu->GetMenuString (i, szText, MF_BYPOSITION);//��ȡ�˵����� 
